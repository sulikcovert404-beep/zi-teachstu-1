...
import { getAttemptResult } from "@/server/services/exam";

export const dynamic = "force-dynamic";

// GET /api/v1/student/exam-attempts/{attempt_id}/result — own result only (spec §27, §18.4)
export const GET = handler(
  async (req: NextRequest, { params }: { params: Promise<{ attemptId: string }> }) => {
    ...
    return ok(await getAttemptResult(tenantId, ctx.userId, attemptId));
  }
);
